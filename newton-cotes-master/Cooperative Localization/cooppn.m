...
Qtrue = diag([qxg qxg qthg qxa qxa qtha]);
% Qtrue = zeros(6);
...
vg = 2;                                         % UGV speed (m/s)
% phi = @(t) -pi()/18;                                 % UGV steering angle (rad)
...
% wx = sqrt(Qtrue)*randn(6, length(t));
wx = mvnrnd(zeros(length(t), 6), Qtrue)';

% Generate simulation data
xt = zeros(6, length(t));
xt(:, 1) = x0;
hodefunpn = @(t, x, u, w, L) h(x, u(t), L) + w;
for k = 2:length(t)
    options = odeset('RelTol', 1e-12, 'AbsTol', 1e-12);
    [tout, xout] = ode45(hodefunpn, [t(k - 1) t(k)], xt(:, k - 1), options, ufcn, wx(:, k), L);
    xt(:, k) = xout(end, :)';
end
...
for k = 2:length(t)
    ...
    Pm = Hxk*reshape(P(:, k), 6, 6)*Hxk' + Q;
%     xm = x(:, k - 1) + h(x(:, k - 1), u(:, k - 1), L)*dt;
    [tout, xout] = ode45(hodefun2, [t(k - 1) t(k)], x(:, k - 1), options, u(:, k - 1), L);
    xm = xout(end, :)';
...
sigth = sqrt(P(36, :));
figure;
plot(t, ex(6, :)*180/pi(), t, 2*sigth, 'g-', t, -2*sigth, 'g-');
