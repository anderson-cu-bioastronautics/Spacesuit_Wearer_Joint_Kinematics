...
char incoming_byte[7400];
char space[1];
char hwbuffer[16384];
void setup() {
HWSERIAL.begin(1000000); // opens serial port, sets data rate to 1MHz bps
...
HWSERIAL.flush();
  ...
  if (HWSERIAL.available() > 0) {
    ...
    Time = millis();
    HWSERIAL.readBytes(incoming_byte,7400);
    ...
    auto data_file = sd.open("IMU1-Calibration-6-13.bin", FILE_WRITE);
    //data_file.write(time_arr,4);
    data_file.write(incoming_byte,7400);
