...
#define HWSERIAL3 Serial7
#define HWSERIAL4 Serial6
#define HWSERIAL5 Serial5
#define HWSERIAL6 Serial4
#define HWSERIAL7 Serial8
#define HWSERIAL8 Serial3
//#define BLOCKLENGTH 16200
#define BLOCKLENGTH 2700
...
char hwbuffer3[BUFFERSIZE];
char hwbuffer4[BUFFERSIZE];
char hwbuffer5[BUFFERSIZE];
// Pin assignments
...
// Increase UART buffers  
  ...
  HWSERIAL3.addMemoryForRead(hwbuffer3, BUFFERSIZE);
  HWSERIAL4.addMemoryForRead(hwbuffer4, BUFFERSIZE);
  HWSERIAL5.addMemoryForRead(hwbuffer5, BUFFERSIZE);
...
// Check if SD card initialized correctly
  ...
  sd.mkdir("Subject2-15-8-21");
  sd.chdir("/Subject2-15-8-21");
}
...
//  memset(incoming_byte2, 0, sizeof(incoming_byte2));
  ...
  if (start_stop == HIGH){
    ...
    HWSERIAL1.clear(); // clear serial ports
    HWSERIAL2.clear();
    HWSERIAL3.clear();
    HWSERIAL4.clear();
    HWSERIAL5.clear();
    ...
    while(HWSERIAL1.available() == 0){}
     ...
     if (HWSERIAL3.available() > 0) {
      HWSERIAL3.readBytes(incoming_byte3,BLOCKLENGTH);
      fWriteData = true;
     }
     //delay(5);
     if (HWSERIAL4.available() > 0) {
      HWSERIAL4.readBytes(incoming_byte4,BLOCKLENGTH);
      fWriteData = true;
      }
      //delay(5);
       if (HWSERIAL5.available() > 0) {
      HWSERIAL5.readBytes(incoming_byte5,BLOCKLENGTH);
      fWriteData = true;
      }
     //delay(5);
    if (fWriteData) {
      if (actIMU1) {
        auto data_file1= sd.open("IMU1-Data.bin", FILE_WRITE);
        data_file1.write(incoming_byte1,BLOCKLENGTH);
        data_file1.close();
      }
      if (actIMU2) {
        auto data_file2 = sd.open("IMU2-Data.bin", FILE_WRITE);
        data_file2.write(incoming_byte2,BLOCKLENGTH);
        data_file2.close();
      }
      if (actIMU3) {
        auto data_file3 = sd.open("IMU3-Data.bin", FILE_WRITE);
        data_file3.write(incoming_byte3,BLOCKLENGTH);
        data_file3.close();
      }
      if (actIMU4) {
        auto data_file4 = sd.open("IMU4-Data.bin", FILE_WRITE);
        data_file4.write(incoming_byte4,BLOCKLENGTH);
        data_file4.close();
      }
      if (actIMU5) {
        auto data_file5 = sd.open("IMU5-Data.bin", FILE_WRITE);
        data_file5.write(incoming_byte5,BLOCKLENGTH);
        data_file5.close();
      }
      if (actIMU6) {
